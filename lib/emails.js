        ...
        <tr>
          <td style="padding:24px;text-align:center;">
            <p style="margin:0;color:#9a9085;font-size:12px;">
              ${isAr ? '© 2024 Stabena — جميع الحقوق محفوظة' : '© 2024 Stabena — Tous droits réservés'}
            </p>
...
export const welcomeEmail = (name, lang = 'fr') => {
  ...
  const content = isAr ? `
    ...
    <div style="background:#f8f5f2;border-radius:10px;padding:20px;margin:20px 0;font-family:Arial,sans-serif;">
      ...
      <p style="margin:6px 0;color:#6b7280;">🎂 أضف تاريخ ميلادك واستلم هدية عيد ميلادك!</p>
    </div>
    <div style="text-align:center;margin-top:28px;">
      ${btn('اكتشف متجرنا →', process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000', STABENA_COLOR)}
    </div>
  ` : `
    ...
    <div style="background:#f8f5f2;border-radius:10px;padding:20px;margin:20px 0;">
      ...
      <p style="margin:6px 0;color:#6b7280;">🎂 Renseignez votre date de naissance pour recevoir un cadeau d'anniversaire !</p>
    </div>
    <div style="text-align:center;margin-top:28px;">
      ${btn('Découvrir la boutique →', process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000', STABENA_COLOR)}
    </div>
  `
  return {
...
export const orderCancelledEmail = (name, orderId, lang = 'fr') => {
  ...
  const content = isAr ? `
    ...
    <div style="text-align:center;margin-top:20px;">
      ${btn('تصفح منتجاتنا →', `${BASE_URL}/products`, STABENA_COLOR)}
    </div>
  ` : `
    <h2 style="color:#2c3e50;font-size:22px;margin:0 0 8px;">
    ...
    <div style="text-align:center;margin-top:20px;">
      ${btn('Continuer mes achats →', `${BASE_URL}/products`, STABENA_COLOR)}
    </div>
  `
  return {
...
// ============================================
// 7. EMAIL D'ANNIVERSAIRE 🎂
// ============================================
export const birthdayEmail = (name, age, discountPercent, code, lang = 'fr') => {
  const isAr = lang === 'ar'
  const BASE_URL = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'

  const content = isAr ? `
    <div style="text-align:center;padding:10px 0 20px;font-family:Arial,sans-serif;">
      <div style="font-size:60px;margin-bottom:8px;">🎂</div>
      <h2 style="color:#2c3e50;font-size:26px;margin:0 0 6px;">
        عيد ميلاد سعيد، ${name}!
      </h2>
      <p style="color:#6b7280;font-size:15px;margin:0;">
        تُكمل اليوم ${age} عاماً — كل عام وأنت بخير ✨
      </p>
    </div>
    ${divider()}
    <div style="background:linear-gradient(135deg,#f0ebe3,#fff8f0);border-radius:16px;padding:28px;margin:20px 0;text-align:center;border:2px dashed #c9a96e;font-family:Arial,sans-serif;">
      <p style="margin:0 0 8px;color:#6b7280;font-size:14px;">هديتك من Stabena</p>
      <p style="margin:0 0 12px;font-size:32px;font-weight:bold;color:#c9a96e;">خصم ${discountPercent}%</p>
      <p style="margin:0 0 16px;color:#6b7280;font-size:13px;">على طلبك التالي بقيمة $80 أو أكثر</p>
      <div style="background:#2c3e50;color:white;padding:12px 24px;border-radius:8px;font-family:monospace;font-size:20px;letter-spacing:4px;display:inline-block;">
        ${code}
      </div>
      <p style="margin:16px 0 0;color:#9a9085;font-size:12px;">
        ⏰ هذا الكود صالح ليوم واحد فقط — استخدمه الآن!
      </p>
    </div>
    <div style="text-align:center;margin-top:24px;">
      ${btn('تسوق الآن →', `${BASE_URL}/products`, '#c9a96e')}
    </div>
    <p style="color:#9a9085;font-size:12px;text-align:center;margin-top:20px;font-family:Arial,sans-serif;">
      أدخل الكود عند إتمام طلبك. صالح لاستخدام واحد فقط.
    </p>
  ` : `
    <div style="text-align:center;padding:10px 0 20px;">
      <div style="font-size:60px;margin-bottom:8px;">🎂</div>
      <h2 style="color:#2c3e50;font-size:26px;margin:0 0 6px;font-family:Georgia,serif;">
        Joyeux anniversaire, ${name} !
      </h2>
      <p style="color:#6b7280;font-size:15px;margin:0;">
        Vous fêtez aujourd'hui vos ${age} ans — que cette année vous apporte bonheur ✨
      </p>
    </div>
    ${divider()}
    <div style="background:linear-gradient(135deg,#f0ebe3,#fff8f0);border-radius:16px;padding:28px;margin:20px 0;text-align:center;border:2px dashed #c9a96e;">
      <p style="margin:0 0 8px;color:#6b7280;font-size:14px;">Votre cadeau Stabena</p>
      <p style="margin:0 0 12px;font-size:32px;font-weight:bold;color:#c9a96e;">${discountPercent}% de réduction</p>
      <p style="margin:0 0 16px;color:#6b7280;font-size:13px;">sur votre prochaine commande de $80 ou plus</p>
      <div style="background:#2c3e50;color:white;padding:12px 24px;border-radius:8px;font-family:monospace;font-size:20px;letter-spacing:4px;display:inline-block;">
        ${code}
      </div>
      <p style="margin:16px 0 0;color:#9a9085;font-size:12px;">
        ⏰ Ce code est valable uniquement aujourd'hui — ne le manquez pas !
      </p>
    </div>
    <div style="text-align:center;margin-top:24px;">
      ${btn('Faire mes achats →', `${BASE_URL}/products`, '#c9a96e')}
    </div>
    <p style="color:#9a9085;font-size:12px;text-align:center;margin-top:20px;">
      Entrez le code lors de votre commande. Valable une seule fois.
    </p>
  `
  return {
    subject: isAr
      ? `🎂 عيد ميلاد سعيد ${name}! هديتك بداخل...`
      : `🎂 Joyeux anniversaire ${name} ! Votre cadeau vous attend...`,
    html: baseLayout(content, lang)
  }
}

// Map status → template
export const getEmailForStatus = (status, name, orderId, items, _total, lang) => {
  switch (status) {
